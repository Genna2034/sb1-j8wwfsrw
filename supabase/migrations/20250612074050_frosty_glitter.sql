...
This migration creates the core database schema for the Emmanuel application, including:

1. Utility Functions
   - update_updated_at_column: Updates the updated_at timestamp on record changes
   - update_notifications_read_at: Updates read_at timestamp when a notification is marked as read

2. Core Tables
   - users: Staff and admin user accounts
   - patients: Patient records with personal and medical information
   - medical_records: Patient medical history entries
   - appointments: Scheduled appointments between staff and patients
   - time_entries: Staff time tracking records
